...
ajzaa = {1: 'الأول', 2: 'الثاني', 3: 'الثالث', 4: 'الرابع', 5: 'الخامس', 6: 'السادس', 7: 'السابع'
...
قال الله تعالى : فأجره حتى يسمع كلام الله  الآية 
    ...
    def show_page(self, page=1):
        ...
        html = html.replace('{joze}', 'الجزء '+ajzaa[self.db.info_page(page)[1]])
        ...
        if len(self.my_aya.keys()) != 0:
            if page in self.my_aya.keys():
                for a in self.my_aya[page]:
                    ...
                    html = re.sub(aya_txt, '<span style="background-color: rgb(255, 245, 177);">{}</span>'.format(aya_txt,), html)
        self.view_quran.load_alternate_html(html, 'file://{}/'.format(asm_path.MOSHAF_DIR,), 'file://{}/'.format(asm_path.MOSHAF_DIR,))
        self.page_id = page
